...
async def handle_back_to_cats(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if not categories:
            ...
            return
        keyboard = [
            [InlineKeyboardButton(cat["name"], callback_data=_shorten_showcat_cb(cat.get('path') or cat.get('name'), 1))]
...
async def courses_callback(update: Update, context: CallbackContext):
    ...
    try:
        ...
        await safe_edit_message(query, "An error occurred while fetching courses. Please try again later.", action_key=getattr(query, 'data', None))from telegram.ext import CommandHandler, CallbackQueryHandler, ConversationHandler, MessageHandler, filters, CallbackContext
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
import logging
from conversation_states import CREATE_CAT_NAME, CREATE_CAT_PARENT
from handlers.db_connection import get_db  # Importing get_db from db_connection.py
