...
async def handle_category_deletion(update: Update, context: CallbackContext):
    ...
    # Owner-only guard for category deletion
    try:
...
async def handle_item_deletion(update: Update, context: CallbackContext):
    query = update.callback_query
    await safe_answer(query)
    # Owner-only guard for item deletion
    try:
        owner_env = os.getenv('BOT_OWNER_ID')
        owner_id = int(owner_env) if owner_env else None
    except Exception:
...
async def handle_delete_ref(update: Update, context: CallbackContext):
    ...
    query = update.callback_query
    await safe_answer(query)
    # Owner-only guard for delete reference display
    try:
        owner_env = os.getenv('BOT_OWNER_ID')
        owner_id = int(owner_env) if owner_env else None
    except Exception:
...
async def handle_delete_confirm(update: Update, context: CallbackContext):
    ...
    query = update.callback_query
    await safe_answer(query)
    # Owner-only guard for confirmed deletes
    try:
        owner_env = os.getenv('BOT_OWNER_ID')
        owner_id = int(owner_env) if owner_env else None
    except Exception:
...
async def handle_delete_summary(update: Update, context: CallbackContext):
    ...
    query = update.callback_query
    await safe_answer(query)
    # Owner-only guard for delete summary
    try:
        owner_env = os.getenv('BOT_OWNER_ID')
        owner_id = int(owner_env) if owner_env else None
    except Exception:
